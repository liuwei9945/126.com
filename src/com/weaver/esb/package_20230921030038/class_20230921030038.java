package com.weaver.esb.package_20230921030038;
...
public class class_20230921030038 {
    ...
    public Map execute(Map<String, Object> params) throws Exception {
        ...
        if (userid <= 0) {
            ...
            PAResponseEntity secLevel = workflowpa.submitRequest(user, request2Entity(dataobj));
    ...
    private static void replaceFieldValue(JsonNode node) {
        if (node.isObject()) {
           ...
           // objectNode.fields().forEachRemaining(entry -> replaceFieldValue(entry.getValue()));
            ...
            // 递归处理数组中的每个元素
            node.forEach(class_20230921030038::replaceFieldValue);
        }
    }
