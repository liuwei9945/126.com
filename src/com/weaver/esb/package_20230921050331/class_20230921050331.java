package com.weaver.esb.package_20230921050331;
...
public class class_20230921050331 {
        ...
        public Map execute(Map<String, Object> params) throws Exception {
            ...
            if (userid <= 0) {
                ...
                //获取传递参数
                WorkflowRequestOperatePA workflowpa = this.getRequestOperatePA();
                User user = new User(userid);

                int requestid=Util.getIntValue(Util.null2String(dataobj.getString("requestId")));
                PAResponseEntity secLevel =null;
                if(requestid<=0)
                {
                    secLevel = workflowpa.doCreateRequest(user, request2Entity(dataobj));
                }
                else {
                    secLevel = workflowpa.submitRequest(user, request2Entity(dataobj));
                }
                ...
                if(requestid<=0)
                {
                    if("SUCCESS".equals(secLevel.getCode().toString()))
                    {
                       // JSONObject.parseObject(secLevel.getData().toString()).get("requestid").toString();
                        ret.put("requestId",  ((Map<String, Object>)secLevel.getData()).get("requestid").toString());
        ...
        private static void replaceFieldValue(JsonNode node) {
            if (node.isObject()) {
                ...
                // 递归处理子节点
              //  objectNode.fields().forEachRemaining(entry -> replaceFieldValue(entry.getClass().toString()));
            } else if (node.isArray()) {
                // 递归处理数组中的每个元素
                node.forEach(class_20230921050331::replaceFieldValue);
            }
        }
